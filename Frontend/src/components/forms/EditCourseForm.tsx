...
const EditCourseForm = ({
  ...
  const [formData, setFormData] = useState<Course>({ ...courseData });
  ...
  useEffect(() => {
    if (courseData) {
      ...
      setVideoList(courseData.videos || []);
    }
  }, [courseData]);
  ...
  const handleOnSubmit = async (e: ChangeEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.put(`${API_SERVICE.courses}/${courseData._id}`, formData, {
        headers: {
      ...
      <div className="flex flex-row gap-3">
        <div className="w-2/3">
          ...
          <TextInput
            ...
            name="title"
            type="text"
            placeholder="Edit React JS Course"
            required
